...
- Six grounding web searches; six papers added to `docs/references.md`. Most directly relevant: STEP (KDD 2022), TNC (ICLR 2021), TS2Vec (AAAI 2022), Adaptive Spatio-Temporal SSL Weather (arxiv 2511.00049).
- `docs/track_3_rl_reuse/README.md` rewritten end-to-end from placeholder to full plan.

### Risheek's directives embedded in the plan

- "Decisions based on results, not assumptions" → every tier has an explicit decision gate.
- Path A guardrails from Codex review embedded:
  1. Exclude cyclic time features from pretext input (otherwise task is trivial).
  2. Pretrain only on train-period data (no leakage).
  3. Validate via downstream RMSE AND train-val gap.
  4. Sample positives heavy on rainy days (mitigate zero-collapse).
- "I'll decide success criterion after first result" → success bar formally locked after Tier 0 + Tier 1 deliver numbers.

### Pending: minor_project_rl access

Folder access was declined. The plan assumes the knowledge-transfer prompt is accurate. If Tier 2 implementation hits ambiguity on TDC pretext details (positive/negative thresholds, projection head size, freeze schedule), Risheek may need to paste specific files from `minor_project_rl` (e.g., `encoder_tdc.py`, `bc_teacher.py`) for verification. Track 3 README open-questions section enumerates the exact details.

### Order of operations for the dissertation push (revised)

1. **Tomorrow:** Items 1 + 2 (per the combined daily trajectory in this file).
2. **Day +1:** Tier 0 baseline + Tier 1 multi-task. Decision gate evaluation. ~3 hours total.
3. **Day +2:** Tier 2 TDC-analog SSL pretraining. ~6 hours.
4. **Day +3 (if time):** Tier 3 backup or Tier 4 ablation + writeup.

### Files modified in this session

- `docs/track_3_rl_reuse/README.md` — full rewrite (placeholder → full plan).
- `docs/references.md` — 6 papers added in "Cited and verified" section.
- `status.md` — change-log entry added.
- `handover.md` — this entry.
- Task list updated: old `Research minor_project_rl` task closed; 5 new Tier 0–4 tasks added.

### Files NOT modified

- All notebooks, models, utils, data remain untouched.
- `plan.md` not touched (no item-map change; Item 3 is still item 3 in the master map. Detailed plan lives in track 3 per the docs policy).
...
- Added a **Session Execution Plan** section to each Item 1 / 2 track README:
  - `docs/track_1a_imd_minimap/README.md` — 6 ordered steps (pre-flight → station selection → graph → dataset → train → evaluate → handover), with validation gates per step.
  - `docs/track_1b_imd_era5_corr/README.md` — 4 ordered steps (align → stats → visualizations → handover).
  - `docs/track_2_regional_ablation/README.md` — 3-phase plan (GEE fetch → cleaning+pivot+EDA → model runs), with explicit fail-safes.
- Added a **combined daily trajectory** for tomorrow above (orchestrates all four tracks across the day with a time table).
- The next session can pick up `handover.md` → daily trajectory → relevant track `README.md` → execute step by step. No re-derivation needed.

### Item 3 status

- Risheek will start Idea 3 (`minor_project_rl` reuse) **fresh in the next message**.
- I requested folder access to `C:\Users\rishe\minor_project_rl` this session; access was declined, which is fine — Risheek will drive the framing in the next message.
- `docs/track_3_rl_reuse/README.md` remains the placeholder with the 5 questions to answer when scoping begins.

### Files modified in this session

- `docs/track_1a_imd_minimap/README.md` — appended Session Execution Plan.
- `docs/track_1b_imd_era5_corr/README.md` — appended Session Execution Plan.
- `docs/track_2_regional_ablation/README.md` — appended Session Execution Plan (3-phase).
- `handover.md` — added combined daily trajectory and this entry.

### Files NOT modified

- All notebooks, models, utils, data remain untouched.
- `status.md` and `plan.md` were not touched in this sub-step (no policy or item-map change; only track-internal additions per `docs/track_usages.md` rules).
...
- Promoted each track doc to its own subdirectory:
  - `docs/track_1a_imd_minimap.md` → `docs/track_1a_imd_minimap/README.md`
  - `docs/track_1b_imd_era5_corr.md` → `docs/track_1b_imd_era5_corr/README.md`
  - `docs/track_2_regional_ablation.md` → `docs/track_2_regional_ablation/README.md`
  - `docs/track_3_rl_reuse.md` → `docs/track_3_rl_reuse/README.md`
- Added `docs/track_usages.md` codifying the documentation policy: where new files go, naming + linking conventions, token-budget reasoning.
- Updated cross-references in `status.md`, `plan.md`, this file, and `docs/references.md` to the new paths.
- The flat `docs/track_*.md` files no longer exist — replaced by `docs/track_*/README.md`.

### Working policy in one line

**Write deep, link shallow.** Track-specific content lives in `docs/track_<id>_<name>/`. Cross-cutting content lives at root. Entry point in any track directory is `README.md`. Full policy: `docs/track_usages.md`.

### What the next session should do FIRST

1. Read `status.md` for the coarse picture.
2. Read `handover.md` (this file) for fresh context.
3. Read `plan.md` for the item map.
4. Read `docs/track_usages.md` ONCE if not already familiar with the policy.
5. Open the relevant `docs/track_<id>_<name>/README.md` for the work area at hand.

### Next session is expected to (per Risheek)

Risheek will use the next prompt to **scope Item 3 (`minor_project_rl` reuse)**. Before that prompt arrives, no implementation work is started. When the prompt arrives:

1. Explore `C:\Users\rishe\minor_project_rl` (skill: Explore-style scan first, then targeted Reads).
2. Answer the five questions in `docs/track_3_rl_reuse/README.md` ("Questions to answer before any reuse decision").
3. Update `docs/track_3_rl_reuse/README.md` with concrete findings and transferable items.
4. Update `status.md` Item 3 line.
5. Update `plan.md` Item 3 section with a concrete plan if reuse is viable.
6. Append a change-log entry to `status.md` and a new section to this file.

### Files modified in this session

- `status.md` — pointer table updated, doc-policy paragraph added, change-log entry added.
- `plan.md` — track-path references updated, doc-policy section added.
- `handover.md` — this entry.
- `docs/track_usages.md` — new file (policy contract).
- `docs/references.md` — pointer paths updated.
- Moved (no content change): `docs/track_1a_imd_minimap.md` → `docs/track_1a_imd_minimap/README.md` and similarly for 1b, 2, 3.

### Files NOT modified

- All notebooks, models, utils, data, and EDA files remain untouched.

---

## 2026-05-29 — Doc restructure (Claude)

### What changed in this session

- Created the initial documentation tree per Risheek's request:
  - `status.md` (slimmed to a coarse master index in root).
  - `plan.md` (root) — master forward-looking plan with item-level summaries.
  - `handover.md` (this file).
  - `docs/track_1a_imd_minimap.md` — Item 1 Track A specifics.
  - `docs/track_1b_imd_era5_corr.md` — Item 1 Track B specifics.
  - `docs/track_2_regional_ablation.md` — Item 2 specifics.
  - `docs/track_3_rl_reuse.md` — Item 3 placeholder.
  - `docs/references.md` — literature and internal doc index.
- No code or data changed.
- Open clarifications list consolidated here from prior `status.md` content.

> _Superseded by the 2026-05-29 per-track-subdirectories entry above. Flat track files were promoted to per-track directories._
...
2. Read `handover.md` (this file) for fresh context.
3. Read `plan.md` for the item map.
4. Open the relevant `docs/track_*.md` for the work area at hand.
...
1. Explore `C:\Users\rishe\minor_project_rl` (skill: Explore-style scan first, then targeted Reads).
2. Answer the five questions in `docs/track_3_rl_reuse.md` ("Questions to answer before any reuse decision").
3. Update `docs/track_3_rl_reuse.md` with concrete findings and transferable items.
4. Update `status.md` Item 3 line.
5. Update `plan.md` Item 3 section with a concrete plan if reuse is viable.
6. Append a change-log entry to `status.md` and a new section to this file.
...
- `status.md` — restructured to slim coarse form.
- `plan.md` — created.
- `handover.md` — created.
- `docs/track_1a_imd_minimap.md` — created.
- `docs/track_1b_imd_era5_corr.md` — created.
- `docs/track_2_regional_ablation.md` — created.
- `docs/track_3_rl_reuse.md` — created.
- `docs/references.md` — created.

### Files NOT modified (still original)

- All notebooks under `experiments/experimentation_notebooks/`.
- All model files under `models/`.
- All data files under `data/`.
- All utils under `utils/`.

---

## 2026-05-29 — Initial audit + future-work roadmap (Claude)

### What was done

- Code-grounded audit of EDA → all 13 experiments → GNN models → data helpers.
- Corrected GPT's "deep research report" on multiple points (graph is radius-based not kNN, GNN loss is plain MSE not seasonal, exp_11 is data prep not modeling, the "RMSE 15.82" across exp_4–10 is a stale baseline echo).
- Verified exp_12 cosmetic-adjustment cells (64+69) do not corrupt saved metrics by tracing cell order.
- Documented scaler/split misalignment as known issue #2.
- Scoped Items 1 and 2 as the next day's work.

### Open follow-ups handed forward

All consolidated above in "Open clarifications waiting on Risheek".

### Files modified

- `status.md` — created with full audit content.

(This entry retained for backreference; superseded by the 2026-05-29 doc-restructure entry above.)
