...
### Session 2 RUNNING on Kaggle (2026-05-30, update)
exp_18 launched on a Kaggle T4 GPU. Two env fixes were needed and applied (repo + the running notebook): (1) PyG auto-install in the bootstrap (`pip install -q torch_geometric` on Kaggle); (2) data path made glob-robust in `env_paths` + the CFG cell, because Kaggle mounted the uploaded (flat) dataset nested under `/kaggle/input/datasets/...` rather than `/kaggle/input/<slug>` — we now glob for `rain_pivot.parquet` under `/kaggle/input` and use its folder. Early run healthy: epoch 1 train 0.390 / val 0.117 recon-MSE (~159 s/epoch), `pretext_best.pt` saving on val improvement. Reminder: a Kaggle Draft (interactive) session's `/kaggle/working` is ephemeral — download `pretext_best.pt` before the session ends, or use Save & Run All. NOT yet finished/downloaded as of this note.
